...
## ⚠️ Important: Dataset Change - Why I Switched to Amazon Reviews
...
### What I Discovered During Analysis

After completing the full data audit in Notebook 1, the following critical issues were identified with the original dataset:

| Issue | Detail |
|-------|--------|
| **Inverted sentiment distribution** | The data audit revealed a split of 68% Positive, 16% Negative and 16% Neutral. This is the opposite of what real e-commerce review data looks like. On genuine review platforms, dissatisfied customers are far more motivated to leave feedback than satisfied ones, so a dataset dominated by Positive reviews raised serious questions about whether it reflected real customer behaviour. |
| **Rating distribution did not match sentiment** | The rating distribution showed 5-star reviews as the most common by a large margin, with counts reaching nearly 50,000. This is consistent with the inflated Positive sentiment but is the reverse of what the Amazon real-world data showed, where 1-star reviews dominated at 13,123. |
| **Models performed unrealistically well** | Every model trained on the original data achieved over 95% accuracy and F1 scores above 0.96. While this might appear impressive, it confirmed that the dataset was too clean and structured to represent a genuine NLP challenge. A model that performs this well on training data rarely generalises to real-world reviews. |
| **No generalisability** | The vocabulary used in the reviews was repetitive and formulaic. Models trained on it would fail to generalise to real customer feedback because the language patterns did not reflect how customers actually write. |
| **Business insights were not actionable** | Topic modelling and EDA on this data produced findings that could not be presented to real stakeholders because they did not reflect genuine customer behaviour or genuine operational problems. |

### The Decision to Switch

The decision was made to replace the original dataset with **real Amazon customer review data** for the following reasons:

**1. Validating the models was the priority**

The core question was: do the machine learning models and the pipeline actually work? Using data with an inverted and implausible sentiment distribution could not answer this. The only way to confirm the pipeline functions correctly in a real-world context was to test it against data that reflects genuine customer behaviour.

**2. Real data produces the expected distribution**

The Amazon dataset confirmed the expected real-world distribution: **68.2% Negative, 27.6% Positive, 4.2% Neutral**. This is consistent with established research on online review platforms where dissatisfied customers are significantly more motivated to leave feedback. The original dataset's 68% Positive distribution was the reverse of this and would have produced a misleading model that was optimised for the wrong majority class.
...
## 2 Weeks Project Plan

| Day | Notebooks | Tasks |
|-----|-----------|-------|
| Week 1 | 01, 02, 03, 04 | Data audit, cleaning, language detection, text preprocessing and feature engineering |
...
- **Account and Billing** issues drive a 92% negative rate - the single highest priority operational problem
- **Returns and Refunds** follow at 90% negative rate requiring immediate process improvement
- **Customer Service** is mentioned 5,998 times in negative reviews - the most cited complaint theme
